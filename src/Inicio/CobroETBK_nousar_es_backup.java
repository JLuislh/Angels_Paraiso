...
import ClassAngels.EmpleadoItem;
...
public class CobroETBK_nousar_es_backup extends javax.swing.JFrame {
    ...
    private void cobrarOrdenET() {
        try {
            ...
            ps = con.prepareStatement("UPDATE ORDENES SET efectivo = " + CantEfectivo.getText() + ",tarjeta = " + Canttarjeta.getText() + ",ESTADO = 2,transferencia = 0.00,CODIGO = "+codigoEmpleado+" where noorden=" + noorden);
            ps.executeUpdate();
            con.close();
            ps.close();
    ...
    private void cobrarOrdenTT() {
        try {
            ...
            Connection con = conecta.getConexion();
            PreparedStatement ps = null;
            ps = con.prepareStatement("UPDATE ORDENES SET efectivo = " + CantEfectivo.getText() + ",tarjeta = 0.00,transferencia = " + Canttarjeta.getText() + ",ESTADO = 2 ,CODIGO = "+codigoEmpleado+" where noorden=" + noorden);
            ps.executeUpdate();
            ps.executeUpdate();
            con.close();
            ps.close();
